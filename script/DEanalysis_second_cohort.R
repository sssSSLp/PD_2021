...
library(ggbeeswarm)
## Read data-------------------------------------------------------------------------------
info <- read.csv("background_second.csv", header=TRUE, row.names=1)
data <- read.csv("readcount_second.csv", header=TRUE, row.names=1)
...
results = as.data.frame(mutate(as.data.frame(res),
                               DE_test=ifelse((res$log2FoldChange)>log2(1) & # FC threshold
                                              ...
                                              ifelse(-(res$log2FoldChange)>log2(1) & # FC threshold
                                                     ...
                                                     "DEG_down", "non-DEG"))),
                        row.names(res))
#write.csv(results, "DEanalysis_result_second.csv", row.names=F)
...
d.norm.OX <- data.frame(d.norm.OX[,47:96], d.norm.OX[,1:46])# sort by group (Ctrl and PD)
...
condition <- rep(c("PD","Ctrl"), c(46,50))
d.norm.Sh <- d.norm2[,order(colnames(d.norm2), decreasing = FALSE)] # sort samples
d.norm.Sh <- data.frame(condition, t(log2(d.norm.Sh[Sharegene,]+1)))
...
PDgene <- c("CHCHD2", "FBXO7", "GAK", "GBA", 
            "GCH1", "LRRK2", "PARK7", "PINK1", "POLG", "SCARB2", "SYNJ1", "VPS35")
condition <- rep(c("PD","Ctrl"), c(46,50))
d.norm.PD <- d.norm2[,order(colnames(d.norm2), decreasing = FALSE)] # sort samples
d.norm.PD <- data.frame(condition, t(log2(d.norm.PD[PDgene,]+1)))
