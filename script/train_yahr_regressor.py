...
class Objective(object):
    ...
    def __call__(self, trial):
        ...
        # trial.suggest_categorical("model_params/etr/n_estimators", [100, 150, 200, 300, 400, 500])
        # trial.suggest_loguniform("model_params/etr/max_depth", 2, 32)
        # trial.suggest_categorical("model_params/etr/min_samples_split", [4, 8, 16])
        # trial.suggest_categorical("model_params/etr/max_features", ['auto', 0.2, 0.4, 0.6, 0.8])
        ...
        for _ in range(self.trial):
            ...
            for i, (train_i, test_i) in enumerate(kfold.split(self.dataset['read_count'].values)):
                model = PdYahrRegressorPipeline(parameters=params)
                ...
                y_pred = model.predict(x_read_count=self.dataset['read_count'].iloc[test_i, :],
            ...
            evaluation = evaluate_regressor(y_gt_values, y_pread_values)
            score = evaluation['spearman_r']
...
def main():
    ...
    train_dataset, test_dataset = setup_helper.setup_yahr_datasets(args, result)
    ...
    for i, (train_i, val_i) in enumerate(kfold.split(train_dataset['read_count'].values)):
        model = PdYahrRegressorPipeline(parameters=best_params)
        ...
        y_pred = model.predict(x_read_count=train_dataset['read_count'].iloc[val_i, :],
    ...
    evaluation = evaluate_regressor(y_gt_values, y_pred_values)
    print("val evaluation")
    pprint.pprint(evaluation, indent=2)
    result['val evaluation'] = evaluation

    val_prediction_df = pd.DataFrame({'y_pred': y_pred_values[:, 0],
                                      'y_gt': y_gt_values[:, 0]},
                                     index=y_gt.index)
    ...
    if test_dataset is not None:
        ...
        y_pred = model.predict(x_read_count=test_dataset['read_count'],
